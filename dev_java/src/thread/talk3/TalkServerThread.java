	...
	public TalkServerThread(TalkServer ts) {
		...
		try {
			...
			for(TalkServerThread tst:ts.globalList) {
				...
				this.send(Protocol.LOGIN+"#"+currentName); //여기에 보내지는 msg는 this의 메세지가 아닌 현재 메세지가 아닌 이전에 들어온 사람의 닉네임를 보내는 것이다.
			}	
		...
		try {
			while(!isStop) {
				...
				case Protocol.ONE:{ //1:1채팅 200
					...
					send(Protocol.ONE+"#"+nickName+"#"+otherName+"#"+msg1); //this.send()라고 안해도 나자신안에 있는 send()일테니 자기자신에게 보내질 거같은데?
				} break;
				case Protocol.MULTI:{ //n:n채팅 201
					String nickName = st.nextToken();
					...
					broadCasting(Protocol.MULTI+"#"+nickName+"#"+message+"#"+fontColor+"#"+imgChoice);
				} break;
				case Protocol.CHANGE:{ //대화명 바꾸기 202
					String nickName = st.nextToken();
