...
public class A {
	...
	void methodA(ZipCodeVO zcVOS[]) {
		...
		zVO = new ZipCodeVO();//다른 클래스가 정의되므로 그 전에 저장해둔다. 그래야 그 주소번지를 유지할 수 있다. 데이터가 존재할때 인스턴스화
		zcVOS[1] = zVO;
	...
	public static void main(String[] args) {
		...
		//메소드 호출시 파라미터로 넘겨서 주소번지를 넘겨서 다른메소드에서 재정의할 수 있다.
		//그 작업을 methodA에서 처리해보자.
		a.methodA(zcVOS); //zcVOS의 주소가 넘어감
		for (int i = 0; i < zcVOS.length; i++) {
			System.out.println("zcVOS["+i+"]="+zcVOS);
		}
	}
}
