package book.chap05;
/*
 * 오라클서버에서 관리하고 있는 회원집합을 활용하기 위해 Member클래스를 설계하였다.
 * 장점: 배열을 사용하지 않으면서도 변수 3개를 각각 관리할 수 있게 되었다.
 * 		심지어 서로 타입이 다르더라도 이것은 가능한 일이다.
 * 		p177, p187, p173 오라클과 자바을 연결해주는 class ****
 * 단점: 변수가 배열이 아니다 그래서 여러개(동시에)를 담을 수 없다. 계속 사라진다. 이전 것이 사라진다. 
 * 		한번에 한 사람에 대한 정보만 담을 수 있다.
 * 
 * 여러사람의 정보를 담을 수는 없나요? - 객체배열을 사용하기 - 문제해결방법
 */
public class Member {
